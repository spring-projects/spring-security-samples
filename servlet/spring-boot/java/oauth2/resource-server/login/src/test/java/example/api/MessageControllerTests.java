...
import example.OAuth2ResourceServerLoginSecurityConfiguration;
...
@Import(OAuth2ResourceServerLoginSecurityConfiguration.class)
@TestPropertySource(properties = "spring.security.oauth2.resourceserver.jwt.jwk-set-uri=http://localhost/.well-known/jwks.json")
class MessageControllerTests {
	...
	void indexGreetsAuthenticatedUser() throws Exception {
		// @formatter:off
		this.mockMvc.perform(get("/api").with(jwt().jwt((jwt) -> jwt.subject("ch4mpy"))))
	...
	void messageCanBeReadWithScopeMessageReadAuthority() throws Exception {
		// @formatter:off
		this.mockMvc.perform(get("/api/message").with(jwt().jwt((jwt) -> jwt.claim("scope", "message:read"))))
	...
	void messageCanNotBeReadWithoutScopeMessageReadAuthority() throws Exception {
		// @formatter:off
		this.mockMvc.perform(get("/api/message").with(jwt()))
