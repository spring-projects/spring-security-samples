...
/**
 * Integration tests for {@link OAuth2ResourceServerLoginApplication}.
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class OAuth2ResourceServerLoginApplicationITests {
	...
	@Test
	void indexWhenAnonymousThenAllows() throws Exception {
		// @formatter:off
		this.mvc.perform(get("/"))
				.andExpect(status().isOk())
				.andExpect(content().string(containsString("OAuth 2.0 Login and Resource Server")));
		// @formatter:on
	}

	@Test
	void authenticatedWhenAnonymousThenRedirectsToLogin() throws Exception {
		// @formatter:off
	...
	void performWhenValidBearerTokenThenAllows() throws Exception {
		// @formatter:off
		this.mvc.perform(get("/api").with(bearerToken(this.noScopesToken)))
				.andExpect(status().isOk())
	...
	void performWhenValidBearerTokenThenScopedRequestsAlsoWork() throws Exception {
		// @formatter:off
		this.mvc.perform(get("/api/message").with(bearerToken(this.messageReadToken)))
				.andExpect(status().isOk())
	...
	void performWhenInsufficientlyScopedBearerTokenThenDeniesScopedMethodAccess() throws Exception {
		// @formatter:off
		this.mvc.perform(get("/api/message").with(bearerToken(this.noScopesToken)))
	...
	void performPostWhenValidBearerTokenThenScopedRequestsAlsoWork() throws Exception {
		// @formatter:off
		this.mvc.perform(post("/api/message").content("example message")
