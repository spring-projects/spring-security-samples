...
public class Saml2LoginIntegrationTests {
	...
	@Test
	void applicationAccessWhenSingleProviderAndUnauthenticatedThenRedirectsToAuthNRequest() throws Exception {
		// @formatter:off
		this.mockMvc.perform(get("http://localhost:8080/some/url"))
	...
	@Test
	void authenticateRequestWhenUnauthenticatedThenRespondsWithRedirectAuthNRequestXML() throws Exception {
		// @formatter:off
		this.mockMvc.perform(get("http://localhost:8080/saml2/authenticate/simplesamlphp"))
				.andExpect(status().is3xxRedirection())
	...
	@Test
	void authenticateRequestWhenRelayStateThenRespondsWithRedirectAndEncodedRelayState() throws Exception {
		// @formatter:off
		MockHttpServletRequestBuilder request = get("http://localhost:8080/saml2/authenticate/simplesamlphp")
	...
	@Test
	void authenticateRequestWhenWorkingThenDestinationAttributeIsSet() throws Exception {
		// @formatter:off
		final String redirectedUrl = this.mockMvc.perform(get("http://localhost:8080/saml2/authenticate/simplesamlphp"))
				.andExpect(status().is3xxRedirection())
				.andReturn()
				.getResponse()
				.getRedirectedUrl();
		MultiValueMap<String, String> parameters = UriComponentsBuilder.fromUriString(redirectedUrl)
						.build(true)
						.getQueryParams();
		// @formatter:on
		String request = parameters.getFirst("SAMLRequest");
		assertThat(request).isNotNull().describedAs("SAMLRequest parameter is missing");
		...
		assertThat(destination).isEqualTo("https://simplesaml-for-spring-saml.cfapps.io/saml2/idp/SSOService.php")
				.describedAs("Destination must match");
		String acsURL = authnRequest.getAssertionConsumerServiceURL();
		assertThat(acsURL).isEqualTo("http://localhost:8080/login/saml2/sso/simplesamlphp")
				.describedAs("AssertionConsumerServiceURL must match");
	}

	@Test
	void authenticateWhenResponseIsSignedThenItSucceeds() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		Response response = buildResponse(assertion);
		signXmlObject(response, getSigningCredential(this.idpCertificate, this.idpPrivateKey, UsageType.SIGNING));
		sendResponse(response, "/").andExpect(authenticated().withUsername(USERNAME));
	}

	@Test
	void authenticateWhenAssertionIsThenItSignedSucceeds() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		Response response = buildResponse(assertion);
	...
	@Test
	void authenticateWhenXmlObjectIsNotSignedThenItFails() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		Response response = buildResponse(assertion);
		sendResponse(response, "/login?error").andExpect(unauthenticated());
	}

	@Test
	void authenticateWhenResponseIsSignedAndAssertionIsEncryptedThenItSucceeds() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		EncryptedAssertion encryptedAssertion = OpenSamlActionTestingSupport.encryptAssertion(assertion,
				decodeCertificate(this.spCertificate));
	...
	@Test
	void authenticateWhenResponseIsSignedAndNameIDisEncryptedThenItSucceeds() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		final EncryptedID nameId = OpenSamlActionTestingSupport.encryptNameId(assertion.getSubject().getNameID(),
				decodeCertificate(this.spCertificate));
	...
	@Test
	void authenticateWhenSignatureKeysDontMatchThenItFails() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		Response response = buildResponse(assertion);
		signXmlObject(assertion, getSigningCredential(this.spCertificate, this.spPrivateKey, UsageType.SIGNING));
		sendResponse(response, "/login?error").andExpect(saml2AuthenticationExceptionMatcher("invalid_signature",
				containsString("Invalid assertion [assertion] for SAML response")));
	}

	@Test
	void authenticateWhenNotOnOrAfterDontMatchThenItFails() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		assertion.getConditions().setNotOnOrAfter(DateTime.now().minusDays(1));
		Response response = buildResponse(assertion);
		signXmlObject(assertion, getSigningCredential(this.idpCertificate, this.idpPrivateKey, UsageType.SIGNING));
		sendResponse(response, "/login?error").andExpect(saml2AuthenticationExceptionMatcher("invalid_assertion",
				containsString("Invalid assertion [assertion] for SAML response")));
	}

	@Test
	void authenticateWhenNotOnOrBeforeDontMatchThenItFails() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
	...
	@Test
	void authenticateWhenIssuerIsInvalidThenItFails() throws Exception {
		Assertion assertion = buildAssertion(USERNAME);
		Response response = buildResponse(assertion);
		response.getIssuer().setValue("invalid issuer");
		signXmlObject(response, getSigningCredential(this.idpCertificate, this.idpPrivateKey, UsageType.SIGNING));
		sendResponse(response, "/login?error").andExpect(unauthenticated()).andExpect(
				saml2AuthenticationExceptionMatcher("invalid_signature", containsString("Invalid signature")));
	}

	private ResultActions sendResponse(Response response, String redirectUrl) throws Exception {
		String xml = toXml(response);
		// @formatter:off
		return this.mockMvc.perform(post("http://localhost:8080/login/saml2/sso/simplesamlphp")
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.param("SAMLResponse", Saml2Utils.samlEncode(xml.getBytes(StandardCharsets.UTF_8))))
				.andExpect(status().is3xxRedirection())
	...
	private Response buildResponse() {
		...
		response.setIssuer(OpenSamlActionTestingSupport
				.buildIssuer("https://simplesaml-for-spring-saml.cfapps.io/saml2/idp/metadata.php"));
		return response;
	}
	...
	private Assertion buildAssertion(String username) {
		...
		assertion.setIssuer(OpenSamlActionTestingSupport
				.buildIssuer("https://simplesaml-for-spring-saml.cfapps.io/saml2/idp/metadata.php"));
		assertion.setSubject(OpenSamlActionTestingSupport.buildSubject(username));
		assertion.setConditions(OpenSamlActionTestingSupport.buildConditions());

		SubjectConfirmation subjectConfirmation = OpenSamlActionTestingSupport.buildSubjectConfirmation();

		// Default to bearer with basic valid confirmation data, but the test can change
		// as appropriate
		subjectConfirmation.setMethod(SubjectConfirmation.METHOD_BEARER);
		final SubjectConfirmationData confirmationData = OpenSamlActionTestingSupport
				.buildSubjectConfirmationData(LOCAL_SP_ENTITY_ID);
	...
	protected Credential getSigningCredential(String certificate, String key, UsageType usageType)
		...
		final PrivateKey privateKey = KeySupport.decodePrivateKey(key.getBytes(StandardCharsets.UTF_8), new char[0]);
	...
	private static ResultMatcher saml2AuthenticationExceptionMatcher(String code, Matcher<String> message) {
		return (result) -> {
